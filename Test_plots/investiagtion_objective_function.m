...
subplot(2,3,1); s = arrayfun(@(b) rotational_energy_new(t_nom, D_nom, b, b2_nom, rho, W_nom, n_nom, Omega_nom), b1_vec);
plot(b1_vec*1000, s/1e6, 'LineWidth', 1.5); xlabel('b1 [mm]'); ylabel('Max Energy [J/kg]'); title('Variation with b1'); grid on;

subplot(2,3,2); s = arrayfun(@(b) rotational_energy_new(t_nom, D_nom, b1_nom, b, rho, W_nom, n_nom, Omega_nom), b2_vec);
plot(b2_vec*1000, s/1e6, 'LineWidth', 1.5); xlabel('b2 [mm]'); ylabel('Max Energy [J/kg]'); title('Variation with b2'); grid on;

subplot(2,3,3); s = arrayfun(@(d) rotational_energy_new(t_nom, d, b1_nom, b2_nom, rho, W_nom, n_nom, Omega_nom), D_vec);
plot(D_vec*1000, s/1e6, 'LineWidth', 1.5); xlabel('D [mm]'); ylabel('Max Energy [J/kg]'); title('Variation with Diameter'); grid on;

subplot(2,3,4); s = arrayfun(@(t) rotational_energy_new(t, D_nom, b1_nom, b2_nom, rho, W_nom, n_nom, Omega_nom), t_vec);
plot(t_vec*1000, s/1e6, 'LineWidth', 1.5); xlabel('t [mm]'); ylabel('Max Energy [J/kg]'); title('Variation with t_{rim}'); grid on;

subplot(2,3,5); s = arrayfun(@(w) rotational_energy_new(t_nom, D_nom, b1_nom, b2_nom, rho, W_nom, n_nom, w), W_vec);
plot(W_vec*60/(2*pi), s/1e6, 'LineWidth', 1.5); xlabel('Speed [RPM]'); ylabel('Max Energy [J/kg]'); title('Variation with \omega'); grid on;
...
s1 = arrayfun(@(b) rotational_energy_new(t_nom, D_nom, b, b2_nom, rho, W_nom, n_nom, Omega_nom), b1_vec);
s2 = arrayfun(@(b) rotational_energy_new(t_nom, D_nom, b1_nom, b, rho, W_nom, n_nom, Omega_nom), b2_vec);
...
function out2 = overal_energy(t_ring, D, b1, b2, rho, W, n, Omega)
    [~, out2, ~] = rotational_energy_new(t_ring, D, b1, b2, rho, W, n, Omega);
end

subplot(2,3,1); s = arrayfun(@(b) overal_energy(t_nom, D_nom, b, b2_nom, rho, W_nom, n_nom, Omega_nom), b1_vec);
plot(b1_vec*1000, s/1e6, 'LineWidth', 1.5); xlabel('b1 [mm]'); ylabel('Max Energy [J]'); title('Variation with b1'); grid on;

subplot(2,3,2); s = arrayfun(@(b) overal_energy(t_nom, D_nom, b1_nom, b, rho, W_nom, n_nom, Omega_nom), b2_vec);
plot(b2_vec*1000, s/1e6, 'LineWidth', 1.5); xlabel('b2 [mm]'); ylabel('Max Energy [J]'); title('Variation with b2'); grid on;

subplot(2,3,3); s = arrayfun(@(d) overal_energy(t_nom, d, b1_nom, b2_nom, rho, W_nom, n_nom, Omega_nom), D_vec);
plot(D_vec*1000, s/1e6, 'LineWidth', 1.5); xlabel('D [mm]'); ylabel('Max Energy [J]'); title('Variation with Diameter'); grid on;

subplot(2,3,4); s = arrayfun(@(t) overal_energy(t, D_nom, b1_nom, b2_nom, rho, W_nom, n_nom, Omega_nom), t_vec);
plot(t_vec*1000, s/1e6, 'LineWidth', 1.5); xlabel('t [mm]'); ylabel('Max Energy [J]'); title('Variation with t_{rim}'); grid on;

subplot(2,3,5); s = arrayfun(@(w) overal_energy(t_nom, D_nom, b1_nom, b2_nom, rho, W_nom, n_nom, w), W_vec);
plot(W_vec*60/(2*pi), s/1e6, 'LineWidth', 1.5); xlabel('Speed [RPM]'); ylabel('Max Energy [J]'); title('Variation with \omega'); grid on;
